const songs = [
    ...
    },
    {
        ...
        lyrics: [
            ...
            { time: 78, text: "Mono Maillot, revolution" }
        ]
    },
    {
        title: "Moñota",
...
// Register songs with global player
if (typeof globalPlayer !== 'undefined') {
    globalPlayer.registerSongs("disco1", songs);
    if (globalPlayer.currentDisco === "disco1" && globalPlayer.currentSongIndex !== null) {
...
function playSong(startTime = audio.currentTime || 0) {
    if (typeof globalPlayer !== "undefined") {
        globalPlayer.playSong("disco1", songIndex, startTime);
    } else {
...
function selectTrack(index, openPanel = true) {
    const isCurrentGlobalTrack = typeof globalPlayer !== "undefined"
        && globalPlayer.currentDisco === "disco1"
...
document.addEventListener("global-player-track-change", (event) => {
    if (event.detail.disco !== "disco1") return;
